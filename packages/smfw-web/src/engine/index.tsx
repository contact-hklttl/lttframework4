...
class Engine {
    ...
    addTemplate(name: string, options: ElementNodeTypes.LoadOptions, map?: EngineTypes.ReplaceOptions) {
        const template = _.cloneDeep(options);
    ...
    createApp() {
        ...
        const pages = themeNames.map((themeName: string, _i: number) => (
            <ThemeProvider key={themeName} theme={createTheme(this.themes[themeName])}>
                <Router>
                    {Drawer && <Drawer />}
                    {this.createPages(themes[themeName])}
                </Router>
            </ThemeProvider>
        ));
